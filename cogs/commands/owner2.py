    ...
    async def global_ban(self, ctx: commands.Context, user: discord.User, reason: str = "Severe violations of Discord's terms of service."):
        ...
        async def confirm(interaction):
            ...
            async def list_success(interaction):
                ...
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Successful Bans [{len(success)}]", color=0x87CEEB, per_page=10),
                    ctx=ctx
                )
            ...
            async def list_failure(interaction):
                ...
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Unsuccessful Bans [{len(failure)}]", color=0x87CEEB, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
    ...
    async def global_kick(self, ctx: commands.Context, user: discord.User, reason: str = "Severe violations of Discord's terms of service."):
        ...
        async def confirm(interaction):
            ...
            async def list_success(interaction):
                ...
                entries = [f"{i+1}. {name}" for i, name in enumerate(success)]
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Successful Kicks [{len(success)}]", color=0x87CEEB, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
            ...
            async def list_failure(interaction):
                ...
                entries = [f"{i+1}. {name}" for i, name in enumerate(failure)]
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Unsuccessful Kicks [{len(failure)}]", color=0x87CEEB, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
    ...
    async def global_timeout(self, ctx: commands.Context, user: discord.User, reason: str = "Severe violations of Discord's terms of service."):
        ...
        async def confirm(interaction):
            ...
            async def list_success(interaction):
                ...
                entries = [f"{i+1}. {name}" for i, name in enumerate(success)]
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Successful Timeouts [{len(success)}]", color=0x87CEEB, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
            ...
            async def list_failure(interaction):
                ...
                entries = [f"{i+1}. {name}" for i, name in enumerate(failure)]
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Unsuccessful Timeouts [{len(failure)}]", color=0x87CEEB, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
    ...
    async def global_nick(self, ctx: commands.Context, user: discord.User, *, name: str):
        ...
        async def confirm(interaction):
            ...
            async def list_success(interaction):
                ...
                entries = [f"{i+1}. {name}" for i, name in enumerate(success)]
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Successful Nickname Change [{len(success)}]", color=0x87CEEB, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
            ...
            async def list_failure(interaction):
                ...
                entries = [f"{i+1}. {name}" for i, name in enumerate(failure)]
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Unsuccessful Nickname Change [{len(failure)}]", color=0x87CEEB, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
    ...
    async def global_clearnick(self, ctx: commands.Context, user: discord.User):
        ...
        async def confirm(interaction):
            ...
            async def list_success(interaction):
                ...
                entries = [f"{i+1}. {name}" for i, name in enumerate(success)]
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Successful Nickname Clear [{len(success)}]", color=0x87CEEB, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
            ...
            async def list_failure(interaction):
                ...
                entries = [f"{i+1}. {name}" for i, name in enumerate(failure)]
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Unsuccessful Nickname Clear [{len(failure)}]", color=0x87CEEB, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
    ...
    async def global_freezenick(self, ctx: commands.Context, user: discord.User, *, name: str):
        ...
        async def confirm(interaction):
            ...
            async def list_success(interaction):
                ...
                entries = [f"{i+1}. {name}" for i, name in enumerate(success)]
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Successful Freezes [{len(success)}]", color=0x87CEEB, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
            ...
            async def list_failure(interaction):
                ...
                entries = [f"{i+1}. {name}" for i, name in enumerate(failure)]
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Unsuccessful Freezes [{len(failure)}]", color=0x87CEEB, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
