...
class HelpCommand(commands.HelpCommand):
  ...
  async def send_bot_help(self, mapping):
    ...
    embed = discord.Embed(
        description=(
         f"**<a:ArrowRed:1448951520077811806> __Start Ariyan X Today__**\n"        
    ...
    embed.set_footer(
      text=f"Requested By {self.context.author} | [Support](https://discord.gg/PkDre8Juhp)",
    )
  ...
  async def send_cog_help(self, cog):
    ...
    paginator = Paginator(source=FieldPagePaginator(
      entries=entries,
      title=f"Ariyan's {cog.qualified_name.title()} ({len(cog.get_commands())})",
