...
############ FUNCTIONS ############
    ...
    async def add(self, ctx, word: str):
        ...
        if await self.is_word_blacklisted(guild_id, word.lower()):
            embed = discord.Embed(title="<:ztick:1448951767990796298> Access Denied",
    ...
    async def bypass_add(self, ctx, target: Union[discord.Member, discord.Role]):
        ...
        if isinstance(target, discord.Member):
            ...
            if await self.is_user_bypassed(guild_id, target.id):
                embed = discord.Embed(
                    description=f"<:zcross:1448951756372443296> | `{target}` is already bypassed.",
                    color=discord.Color.from_rgb(255, 0, 0)
                )
                await ctx.reply(embed=embed)
                return
    ...
    async def command_error(self, ctx, error):
        if isinstance(error, commands.CommandError):
            if not isinstance(error, commands.CommandOnCooldown):
                embed = discord.Embed(
                    description="<:zcross:1448951756372443296> | An error occurred while processing the command. Make sure you have **Administrator** permission.",
                    color=discord.Color.from_rgb(255, 0, 0)
                )
                await ctx.reply(embed=embed)