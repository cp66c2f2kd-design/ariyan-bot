...
class AutoResponder(commands.Cog):
    ...
    async def _create(self, ctx, name, *, message):
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            async with db.execute("SELECT COUNT(*) FROM autoresponses WHERE guild_id = ?", (ctx.guild.id,)) as cursor:
                ...
                if count >= 20:
                    return await ctx.reply(embed=discord.Embed(title="<:zcross:1448951756372443296> Error!",
            ...
            await db.commit()
            await ctx.reply(embed=discord.Embed(title="<:ztick:1448951767990796298> Success",
    ...
    async def _config(self, ctx):
        ...
        embed = discord.Embed(color=0x87CEEB, title=f"Autoresponders in {ctx.guild.name}")
