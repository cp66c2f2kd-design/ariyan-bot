    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if log_channel:
            embed = discord.Embed(
                ...
                description=f"**User**: [{self.user}](https://discord.com/users/{self.user.id})\n**User Mention**: {self.user.mention}\n** ID**: {self.user.id}\n\n** Added By**: [{self.author.display_name}](https://discord.com/users/{self.author.id})\n<a:timer:1329404677820911697>**Expiry Time**: {expiry_text}\n<:zdil:1448949605676093540> **Timestamp**: {expiry_timestamp}\n\n<a:premium:1204110058124873889> **Tier**: **{self.values[0].upper()}**",
                color=0x87CEEB
            )
            embed.set_thumbnail(url=self.user.avatar.url if self.user.avatar else self.user.default_avatar.url)
            await log_channel.send("<#1396794297386532978>",embed=embed)
            

        
        embed = discord.Embed(description=f"**Added Global No Prefix**:\n**<:zHuman:1448951509235531869> User**: **[{self.user}](https://discord.com/users/{self.user.id})**\n**<:zyrox_mention:1448949481776222218> User Mention**: {self.user.mention}\n**<:zyroxsys:1448949469650620426> User ID**: {self.user.id}\n\n__**Additional Info**__:\n**<:zyroxhammer:1448949447617806458> Added By**: **[{self.author.display_name}](https://discord.com/users/{self.author.id})**\n**<:zyrox_time:1448949493012889610> Expiry Time:** {expiry_text}\n**<:boost:1448966463586041906> Timestamp:** {expiry_timestamp}", color=0x87CEEB)
...
class NoPrefix(commands.Cog):
                ...
                for user_id in expired_users:
                    ...
                    if user:
                        ...
                        if log_channel:
                            ...
                            await log_channel.send("<#1396794297386532978>", embed=embed_log)
                        bot = self.client
                        ...
                        embed = discord.Embed(
                            description=f"<:icons_warning:1448949538944716922> Your No Prefix status has **Expired**. You will now require the prefix to use commands.",
                            color=0x87CEEB
                        )
                        embed.set_author(name="No Prefix Expired", icon_url=user.avatar.url if user.avatar else user.default_avatar.url)
                        
                        embed.set_footer(text="Ariyan - No Prefix, Join support to regain access.")
    ...
    async def np_add(self, ctx, user: discord.User):
        ...
        embed = discord.Embed(title="Select No Prefix Duration", description="**Choose the duration for how long no-prefix should be enabled for this user:**", color=0x87CEEB)
    ...
    async def np_remove(self, ctx, user: discord.User):
        ...
        if log_channel:
            ...
            embed_log.set_thumbnail(url=user.display_avatar.url if user.avatar else user.default_avatar.url)
            embed_log.set_footer(text="No Prefix Removal Log")
            await log_channel.send("<@870179991462236170>", embed=embed_log)
        ...
        async with aiosqlite.connect('db/np.db') as db:
            ...
            embed = discord.Embed(
                ...
                description=(
                    f"**User**: [{user}](https://discord.com/users/{user.id})\n"
                    f"**User ID**: {user.id}\n\n"
                    f"**<:zyrox_time:1448949493012889610> Expiry**: {expire_time} ({expire_timestamp})"
                ),
                color=0x87CEEB
            )
    ...
    async def on_member_update(self, before, after):
        if before.premium_since is None and after.premium_since is not None:
            ...
            if not await self.is_user_in_np(after.id):
                ...
                message = await log_channel.send("<@870179991462236170>", embed=embed)
    ...
    async def add_np(self, user, duration):
        ...
        embed = discord.Embed(
                            ...
                            description=f"You've been credited 2 months of global No Prefix for boosting our Partnered Servers. You can now use my commands without prefix. If you wish to remove it, please reach out [Support Server](https://discord.gg/PkDre8Juhp).",
                            color=0x87CEEB
                        )
        try:
    ...
    async def remove_np(self, user):
        ...
        embed= discord.Embed(title="<:icons_warning:1448949538944716922> Global No Prefix Expired",
                        description=f"Hey {user.mention}, your global no prefix has expired!\n\n__**Reason:**__ Unboosting our partnered Server.\nIf you think this is a mistake then please reach out [Support Server](https://discord.gg/PkDre8Juhp).",
                        color=0x87CEEB)
    ...
    async def np_reset(self, ctx):
        ...
        async def yes_callback(interaction):
            ...
            success_embed = discord.Embed(
                title="<:ztick:1448951767990796298> No-Prefix Reset Complete",
            ...
            if log_channel:
                ...
                await log_channel.send("<#1396794297386532978>", embed=log_embed)
