...
class Welcomer(commands.Cog):
    ...
    async def greet_setup(self, ctx):
        ...
        if row:
            error = discord.Embed(description=f"A welcome message has already been set in {ctx.guild.name}. Use `{ctx.prefix}greet reset` to reconfigure.", color=0x87CEEB)
    ...
    async def simple_setup(self, ctx):
        ...
        async def submit_callback(interaction):
            ...
            if message_content:
                ...
                await interaction.response.send_message("<:ztick:1448951767990796298>> Welcome message setup completed!")
    ...
    color=discord.Color(embed_data["color"]) if embed_data["color"] else discord.Color(0x2f3136)
        ...
        async def submit_callback(interaction):
            ...
            await interaction.response.send_message("<:ztick:1448951767990796298>> Embed welcome message setup completed!")
    ...
    async def greet_reset(self, ctx):
        ...
        if not is_set_up: 
            error = discord.Embed(description=f"No welcome message has been set for {ctx.guild.name}! Please set a welcome message first using `{ctx.prefix}greet setup`", color=0x87CEEB)
        ...
        async def yes_button_callback(interaction):
            ...
            embed.color = discord.Color(0x87CEEB)
            embed.title = "<:ztick:1448951767990796298>> Success"
    ...
    async def greet_channel(self, ctx):
        ...
        def generate_view(page):
            ...
            async def select_callback(interaction: discord.Interaction):
                ...
                await ctx.send(f"<:ztick:1448951767990796298>> Welcome channel has been set to {selected_channel.mention}")
    ...
    async def greet_test(self, ctx):
        ...
        if not welcome_channel:
            error2 = discord.Embed(description=f"Welcome channel not set or invalid. Use `{ctx.prefix}greet channel` to set one.", color=0x87CEEB)
