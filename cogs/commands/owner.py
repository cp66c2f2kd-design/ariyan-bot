...
class Owner(commands.Cog):
    ...
    async def staff_add(self, ctx, user: discord.User):
        if user.id in self.staff:
            sonu = discord.Embed(title="<:zwarning:1448949627712966717>  Access Denied", description=f"{user} is already in the staff list.", color=0x87CEEB)
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                await db.commit()
            sonu2 = discord.Embed(title="<:ztick:1448951767990796298> Success", description=f"Added {user} to the staff list.", color=0x87CEEB)
    ...
    async def staff_remove(self, ctx, user: discord.User):
        if user.id not in self.staff:
            sonu = discord.Embed(title="<:zwarning:1448949627712966717> Access Denied", description=f"{user} is not in the staff list.", color=0x87CEEB)
            await ctx.reply(embed=sonu, mention_author=False)
        else:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                await db.commit()
                sonu2 = discord.Embed(title="<:ztick:1448951767990796298> Success", description=f"Removed {user} from the staff list.", color=0x87CEEB)
            await ctx.reply(embed=sonu2, mention_author=False)
    ...
    async def staff_list(self, ctx):
        if not self.staff:
            ...
            staff_display = "\n".join(member_list)
            sonu = discord.Embed(title="<:ztick:1448951767990796298> Ariyan Staffs", description=f"\n{staff_display}", color=0x87CEEB)
    ...
    async def _slist(self, ctx):
        ...
        paginator = Paginator(source=DescriptionEmbedPaginator(
            entries=entries,
            description="",
            title=f"Guild List of Ariyan [{len(self.client.guilds)}]",
            color=0x87CEEB,
            per_page=10),
            ctx=ctx)
    ...
    async def _restart(self, ctx: Context):
        await ctx.reply("<:ztick:1448951767990796298> | **Successfully Restarting Ariyan It Takes 10 seconds**")
        restart_program()
    ...
    async def own_list(self, ctx):
        ...
        paginator = Paginator(source=DescriptionEmbedPaginator(
            entries=entries,
            title=f"Ariyan Owners [{len(nplist)}]",
            description="",
            per_page=10,
            color=0x87CEEB),
                              ctx=ctx)
    ...
    async def bdg(self, ctx):
        if ctx.invoked_subcommand is None:
            embed = discord.Embed(description='Invalid `bdg` command passed. Use `add` or `remove`.', color=0x87CEEB)
            await ctx.send(embed=embed)
    ...
    async def add(self, ctx, member: discord.Member, badge: str):
        ...
        if badge in BADGE_URLS or badge == 'bug' or badge == 'all':
            if badge == 'all':
                ...
                embed = discord.Embed(description=f"All badges added to {member.mention}.", color=0x87CEEB)
                await ctx.send(embed=embed)
            else:
                success = add_badge(user_id, badge)
                if success:
                    embed = discord.Embed(description=f"Badge `{badge}` added to {member.mention}.", color=0x87CEEB)
                else:
                    embed = discord.Embed(description=f"{member.mention} already has the badge `{badge}`.", color=0x87CEEB)
                await ctx.send(embed=embed)
        else:
            embed = discord.Embed(description=f"Invalid badge: `{badge}`", color=0x87CEEB)
            await ctx.send(embed=embed)
    ...
    async def remove(self, ctx, member: discord.Member, badge: str):
        ...
        if badge in BADGE_URLS or badge == 'bug' or badge == 'all':
            if badge == 'all':
                ...
                embed = discord.Embed(description=f"All badges removed from {member.mention}.", color=0x87CEEB)
                await ctx.send(embed=embed)
            else:
                success = remove_badge(user_id, badge)
                if success:
                    embed = discord.Embed(description=f"Badge `{badge}` removed from {member.mention}.", color=0x87CEEB)
                else:
                    embed = discord.Embed(description=f"{member.mention} does not have the badge `{badge}`.", color=0x87CEEB)
                await ctx.send(embed=embed)
        else:
            embed = discord.Embed(description=f"Invalid badge: `{badge}`", color=0x87CEEB)
            await ctx.send(embed=embed)
