    ...
    async def add(self, ctx, role: discord.Role):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            embed = discord.Embed(title="<:ztick:1448951767990796298> Success",
                                  description=f"VC role {role.mention} added for this guild.", color=0x87CEEB)
            await ctx.reply(embed=embed)
            ...
            async with db.execute('SELECT role_id FROM vcroles WHERE guild_id = ? AND role_id = ?', (ctx.guild.id, role.id)) as cursor:
                ...
                if not row:
                    embed = discord.Embed(title="<:zcross:1448951756372443296> Error",
                                          description="Given role is not set in VC role.", color=0x87CEEB)
                    await ctx.send(embed=embed)
                    return
            await db.execute('DELETE FROM vcroles WHERE guild_id = ? AND role_id = ?', (ctx.guild.id, role.id))
            await db.commit()
            embed = discord.Embed(title="<:ztick:1448951767990796298> Success",
                                  description=f"VC role {role.mention} removed for this guild.", color=0x87CEEB)
            await ctx.send(embed=embed)
    ...
    async def config(self, ctx):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT role_id FROM vcroles WHERE guild_id = ?', (ctx.guild.id,)) as cursor:
                ...
                if not row:
                    embed = discord.Embed(title="<:zcross:1448951756372443296> Error",
                                          description="VC role is not set in this guild.", color=0x87CEEB)
                    await ctx.send(embed=embed)
                    return
                role = ctx.guild.get_role(row[0])
                embed = discord.Embed(title="VC Role Configuration",
                                      description=f"Current VC role in this guild is {role.mention}.", color=0x87CEEB)
            ...
            async with aiosqlite.connect(self.db_path) as db:
                async with db.execute('SELECT role_id FROM vcroles WHERE guild_id = ?', (member.guild.id,)) as cursor:
                    ...
                    if after.channel and role not in member.roles:
                        await self.add_role_with_retry(member, role, reason="Member Joined VC | Zyrox Invcrole")
                    elif not after.channel and role in member.roles:
                        await self.remove_role_with_retry(member, role, reason="Member Left VC | Zyrox Invcrole")
