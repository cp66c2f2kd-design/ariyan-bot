...
class MapView(ui.View):
    ...
    def get_coordinates(self, location):
        ...
        try:
            headers = {'User-Agent': 'Zyrox Bot (https://sorrylinknahithi)'}
    ...
    async def update_embed(self, interaction: discord.Interaction):
        if self.latitude is None or self.longitude is None:
            ...
            return
        embed = discord.Embed(title=f" Map of {self.location}", color=0x87CEEB)
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user != self.ctx.author:
            await interaction.response.send_message(
                embed=discord.Embed(description="Sorry only the requested author can control this", color=0x87CEEB),
                ephemeral=True
            )
            return False
        return True
