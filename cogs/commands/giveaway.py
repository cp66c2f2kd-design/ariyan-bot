...
class Giveaway(commands.Cog):
        ...
        try:
            ...
            if channel is not None:
                try:
                    ...
                    await message.edit(content="<a:TADAA:1448966368044126249> **GIVEAWAY ENDED** <a:TADAA:1448966368044126249>", embed=embed)
                    await message.reply(f"<a:TADAA:1448966368044126249> Congrats {winner}, you won **{giveaway[5]}!**, Hosted by <@{int(giveaway[3])}>")
                    await self.cursor.execute("DELETE FROM Giveaway WHERE message_id = ? AND guild_id = ?", (message.id, message.guild.id))
                    await self.connection.commit()
    ...
    async def gstart(self, ctx,
        ...
        if len(g_list) >= 5:
            embed = discord.Embed(title="<:zwarning:1448949627712966717> Access Denied",
                                  description=f"You can only host upto 5 giveaways in this Guild.", color=0x87CEEB)
            message = await ctx.send(embed=embed)
            await asyncio.sleep(5)
            await message.delete()
            return
        ...
        if converted / 60 >= 50400:
            embed = discord.Embed(title="<:zwarning:1448949627712966717> Access Denied",
                                  description=f"Time cannot exceed 31 days!", color=0x87CEEB)
            message = await ctx.send(embed=embed)
            await asyncio.sleep(5)
            await message.delete()
            return
        ...
        if converted == -1:
            embed = discord.Embed(title="<:zcross:1448951756372443296> Error",
                                  description=f"Invalid time format", color=0x87CEEB)
            message = await ctx.send(embed=embed)
            await asyncio.sleep(5)
            await message.delete()
            return
        ...
        embed = discord.Embed(title=f"<a:TADAA:1448966368044126249> {prize}",
                              description=f"<a:ArrowRed:1448951520077811806> Winner(s): **{winners}**\n<a:ArrowRed:1448951520077811806> Hosted by {ctx.author.mention}\n<a:ArrowRed:1448951520077811806> Ends <t:{round(ends)}:R> (<t:{round(ends)}:f>)\n\n<a:ArrowRed:1448951520077811806> React with <a:TADAA:1448966368044126249> to participate!", color=0x87CEEB)
        ...
        message = await ctx.send("<a:TADAA:1448966368044126249> **GIVEAWAY** <a:TADAA:1448966368044126249>", embed=embed)
        try:
        ...
        await message.add_reaction("<a:TADAA:1448966368044126249>")
        await self.connection.commit()
    ...
    async def gend(self, ctx, message_id = None):
        if message_id:
            try:
                ...
                embed = discord.Embed(title="<:zwarning:1448949627712966717> Access Denied",
                                      description="Invalid message ID provided.", color=0x87CEEB)
                message = await ctx.send(embed=embed)
                await asyncio.sleep(5)
                await message.delete()
                return
        ...
        if message_id is not None:
            ...
            if re is None:
                embed = discord.Embed(title="<:zcross:1448951756372443296> Error",
                                      description=f" The giveaway was not found.", color=0x87CEEB)
                message = await ctx.send(embed=embed)
                await asyncio.sleep(5)
                await message.delete()
                return
            ...
            if len(users) < 1:
                await ctx.send(f"<:ztick:1448951767990796298> Successfully Ended the giveaway in <#{int(re[6])}>")
            ...
            if int(ctx.channel.id) != int(re[6]):
                await ctx.send(f"<a:TADAA:1448966368044126249> Successfully ended the giveaway in <#{int(re[6])}>")
    ...
    async def greroll(self, ctx, message_id: typing.Optional[int] = None):
        ...
        if re is not None:
            embed = discord.Embed(title="<:zwarning:1448949627712966717> Access Denied",
                                  description=f"The giveaway is currently running. Please use the `gend` command instead to end the giveaway.", color=0x87CEEB)
            message = await ctx.send(embed=embed)
            await asyncio.sleep(5)
            await message.delete()
            return
    ...
    async def glist(self, ctx):
        ...
        if not giveaways:
            embed = discord.Embed(description=" No ongoing giveaways.", color=0x87CEEB)
            await ctx.send(embed=embed)
            return

        embed = discord.Embed(title="Ongoing Giveaways", color=0x87CEEB)
