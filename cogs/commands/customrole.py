...
class Customrole(commands.Cog):
    ...
    async def add_role(self, *, role_id: int, member: discord.Member):
        if member.guild.me.guild_permissions.manage_roles:
            ...
            await member.add_roles(role, reason="Zyrox Customrole | Role Added")
        else:
    ...
    async def remove_role(self, *, role_id: int, member: discord.Member):
        if member.guild.me.guild_permissions.manage_roles:
            role = discord.Object(id=role_id)
            await member.remove_roles(role, reason="Zyrox Customrole | Role Removed")
        else:
            raise discord.Forbidden("Bot does not have permission to manage roles.")
    ...
    async def on_message(self, message: discord.Message):
        ...
        if result:
            ...
            if reqrole is None:
                await message.channel.send("<:zwarning:1448949627712966717> The required role is not set up in this server. Please set it up using `setup reqrole`.")
                return

            
            if reqrole not in message.author.roles:
                await message.channel.send(embed=discord.Embed(description=f"<:zwarning:1448949627712966717> You need the {reqrole.mention} role to use this command.", color=0x000000))
                return
