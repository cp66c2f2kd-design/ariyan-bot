...
class RoleInfoView(View):
  ...
  @discord.ui.button(label='Show Permissions',  emoji="<:codebase:1448951697853386826> ", style=discord.ButtonStyle.secondary)
  ...
  async def show_overwrites(self, interaction: discord.Interaction, button: Button):
      ...
      for target, perms in self.channel.overwrites.items():
          ...
          overwrites.append(f"**For {target.name}**\n" +
                            "\n".join(f"  * **{perm}:** {'<:ztick:1448951767990796298>>' if value else '<:zcross:1448951756372443296>' if value is False else '⛔'}" for perm, value in permissions.items()))
      ...
      embed.set_footer(text="<:ztick:1448951767990796298>> = Allowed, <:zcross:1448951756372443296> = Denied, ⛔ = None")
      await interaction.response.send_message(embed=embed, ephemeral=True)
...
class Extra(commands.Cog):
  ...
  async def uptime(self, ctx):
      ...
      embed = discord.Embed(title=f"Ariyan Manager Uptime", color=self.color)
      embed.add_field(name="__UTC__", value=f"<:zwarning:1448949627712966717> {uptime_string}\n\n", inline=False)
      embed.add_field(name="__Online Duration__", value=f"<:uptime:1398280366501920829> {uptime_duration_string}", inline=False)
  ...
  async def serverinfo(self, ctx):
        embed = discord.Embed(color=0x87CEEB).set_author(
        ...
        embed.add_field(
            name="**__About__**",
            value=f"**Name : ** {ctx.guild.name}\n**ID :** {ctx.guild.id}\n**Owner <:king:1448951721479901334> :** {ctx.guild.owner} (<@{ctx.guild.owner_id}>)\n**Created At : ** {c_at}\n**Members :** {len(ctx.guild.members)}",
            inline=False
        )
        ...
        if ctx.guild.features:
            features = "\n".join([f"<:ztick:1448951767990796298>>: {feature[:1].upper() + feature[1:].lower().replace('_', ' ')}" for feature in ctx.guild.features])
            embed.add_field(
                name="**__Features__**",
        ...
        embed.add_field(
            ...
            value=f"Level: {ctx.guild.premium_tier} [<:boost:1448966463586041906>{ctx.guild.premium_subscription_count} boosts]",
            inline=False
        )
...
**ID:** {member.id}
**Nickname:** {nickk}
**Bot?:** {'<:ztick:1448951767990796298>> Yes' if member.bot else '<:zcross:1448951756372443296> No'}
**Badges:** {badges}
...
**Color:** {member.color if member.color else '99aab5'}
  ...
  async def boosts(self, ctx):
    await ctx.send(
      embed=discord.Embed(title=f"<:boost:1448966463586041906> Boosts Count Of {ctx.guild.name}",
  ...
  async def ping(self, ctx: commands.Context):
    ...
    initial_embed = discord.Embed(
        ...
        description="Calculating response times, please wait.",
        color=0x2f3136 # Neutral dark color
    )
    msg = await ctx.send(embed=initial_embed)
    end_time = time.monotonic()

    # 2. Calculate all latencies
    ...
    try:
        async with aiosqlite.connect("db/afk.db") as db:
            ...
            db_latency = f"{round(db_latency_ms)}ms"
    except Exception as e:
        print(f"Database latency check failed: {e}")

    # 3. Set embed color based on total latency
    total_latency = bot_latency + api_latency
    ...
    # 4. Create the final, detailed embed without emojis
    ...
    final_embed.set_footer(text=f"Requested by {ctx.author.name}", icon_url=ctx.author.avatar.url if ctx.author.avatar else None)
    final_embed.timestamp = ctx.message.created_at

    # 5. Edit the original message to show the final embed
  ...
  async def report(self, ctx, *, bug):
    ...
    confirm_embed = discord.Embed(title="<:ztick:1448951767990796298> Bug Reported",
