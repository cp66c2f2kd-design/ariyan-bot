...
class ShowRules(discord.ui.View):
    ...
    async def show_rules(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(title="Enabled Automod Rules", description=enabled_rules, color=0xFF000)
...
class Automod(commands.Cog):
    ...
    async def enable(self, ctx):
        ...
        if ctx.author != ctx.guild.owner and ctx.author.top_role.position < ctx.guild.me.top_role.position:
            embed = discord.Embed(title="<:zcross:1448951756372443296> Access Denied", description="Your top role must be at the **same** position or **higher** than my top role.", color=0x87CEEB)
        ...
        if await self.is_automod_enabled(guild_id):
            embed=discord.Embed(title=f"Automod Settings for {ctx.guild.name}", description=f"**<:zcross:1448951756372443296> Your Server already has Automoderation Enabled.**\n\nCurrent Status: <:Enable:1448949527846322296> Enabled\nTo Disable use `{ctx.prefix}automod disable`", color=0x87CEEB)
        ...
        embed = discord.Embed(title=f"{ctx.guild.name}'s Automod Setup", color=0x87CEEB)
        embed.set_thumbnail(url=self.bot.user.avatar.url)
        embed.description = "\n".join([f"<:Disable:1448949515787964417> : {event}" for event in events])
    ...
    async def enable_automod(self, ctx, guild_id, selected_events, interaction):
        ...
        embed = discord.Embed(title="Automod Enabled Successfully", color=0x87CEEB)
        embed.description = "\n".join([f"<:Enable:1448949527846322296> : {event}" for event in selected_events] +
                                       [f"<:zcross:1448951756372443296><:ztick:1448951767990796298> : {event}" for event in ["Anti spam", "Anti caps", "Anti link", "Anti invites", "Anti mass mention", "Anti emoji spam", "Anti NSFW link"] if event not in selected_events])
        ...
        async def enable_logging_callback(interaction):
            ...
            try:
                for channel in interaction.guild.channels:
                    if channel.name == "zyrox-automod":
                        await interaction.response.send_message(f"A logging channel with the name \"zyrox-automod\" already exists.", ephemeral=True)
                        return
                log_channel = await interaction.guild.create_text_channel("zyrox-automod", overwrites=overwrites)
    ...
    async def punishment(self, ctx):
        ...
        embed = discord.Embed(title=f"Current Automod Punishments for {ctx.guild.name}", color=0x87CEEB)
        punishment_map = {}
        ...
        async def select_callback(interaction):
            ...
            for punishment in ["Mute", "Kick", "Ban"]:
                ...
                async def punishment_callback(button_interaction, selected_events=selected_events, punishment=punishment):
                    ...
                    updated_embed = discord.Embed(title=f"Updated Automod Punishments for {ctx.guild.name}", color=0x87CEEB)
    ...
    async def ignore_channel(self, ctx, channel: discord.TextChannel):
        ...
        async with aiosqlite.connect("db/automod.db") as db:
            ...
            if await cursor.fetchone() is not None:
                embed = discord.Embed(title="__Channel Already Whitelisted!__", description=f"<:zcross:1448951756372443296> The channel {channel.mention} is already in the ignore list.\n\n➜ Use **{ctx.prefix}automod unignore channel {channel.mention}** to remove it.", color=0x87CEEB)
            ...
            success = discord.Embed(title="<:ztick:1448951767990796298> Channel Whitelisted", description=f"The channel {channel.mention} has been added to the ignore list \n\n➜ Use `{ctx.prefix}automod ignore show` to view the ignore list.", color=0x87CEEB)
    ...
    async def ignore_role(self, ctx, role: discord.Role):
        ...
        async with aiosqlite.connect("db/automod.db") as db:
            ...
            if await cursor.fetchone() is not None:
                embed = discord.Embed(title="__Role Already Whitelisted!__", description=f"<:zcross:1448951756372443296> The role {role.mention} is already in the ignore list.\n\n➜ Use **{ctx.prefix}automod unignore role {role.mention}** to remove it.", color=0x87CEEB)
                embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
                await ctx.send(embed=embed)
                return
            ...
            success = discord.Embed(title="<:ztick:1448951767990796298> Role Whitelisted", description=f"The role {role.mention} has been added to the ignore list \n\n➜ Use `{ctx.prefix}automod ignore show` to view the ignore list.", color=0x87CEEB)
            success.set_thumbnail(url=self.bot.user.avatar.url)
            success.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                   icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
    ...
    async def ignore_show(self, ctx):
        ...
        embed = discord.Embed(title="Ignored Channels & Roles for Automod", color=0x87CEEB)
    ...
    async def ignore_reset(self, ctx):
        ...
        async with aiosqlite.connect("db/automod.db") as db:
            ...
            await db.commit()
        embed=discord.Embed(title=f"Automod Settings for {ctx.guild.name}", description=f"** <:ztick:1448951767990796298> | All ignored channels and roles have been reset!**\n\nTo view current Automod settings use `{ctx.prefix}automod config`", color=0x87CEEB)
        embed.set_thumbnail(url=self.bot.user.avatar.url)
        embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
               icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
        await ctx.send(embed=embed)
    ...
    async def unignore_channel(self, ctx, channel: discord.TextChannel):
        ...
        if result.rowcount > 0:
            embed = discord.Embed(title="<:ztick:1448951767990796298> Success", description=f"{channel.mention} has been removed from the automod ignore list.", color=0x87CEEB)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
        else:
            embed = discord.Embed(title="<:zcross:1448951756372443296> Error", description=f"{channel.mention} is not in the automod ignore list.", color=0x87CEEB)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
    ...
    async def unignore_role(self, ctx, role: discord.Role):
        ...
        if result.rowcount > 0:
            embed = discord.Embed(title="<:ztick:1448951767990796298> Success", description=f"{role.mention} has been removed from the automod ignore list.", color=0x87CEEB)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
        else:
            embed = discord.Embed(title="<:zcross:1448951756372443296> Error", description=f"{role.mention} is not in the automod ignore list.", color=0x87CEEB)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
    ...
    async def config(self, ctx):
        ...
        current_punishments = await self.get_current_punishments(guild_id)
        embed = discord.Embed(title=f"Enabled Automod Events & their punishment type for {ctx.guild.name}", color=0x87CEEB)
    ...
    async def logging(self, ctx, channel: discord.TextChannel):
        ...
        async with aiosqlite.connect("db/automod.db") as db:
            ...
            await db.commit()
            embed=discord.Embed(title=f"Automod Settings for {ctx.guild.name}", description=f"**<:ztick:1448951767990796298> | Automoderation Logging channel set to {channel.mention}.**\n\n➜ Use `{ctx.prefix}automod config` to view current Automod settings.", color=0x87CEEB)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                   icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
        await ctx.send(embed=embed)
