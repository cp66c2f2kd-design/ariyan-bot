    ...
    async def invites(self, ctx, member: discord.Member = None):
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"**Left:** `{left}`\n"
                f"**Rejoins:** `{rejoin}`\n\n"
                f"{EMOJI_INVITE} **Get Zyrox Premium Lifetime [Join Support Here](https://discord.gg/PkDre8Juhp)**"
            )
        )
        embed.set_thumbnail(url=member.avatar.url if member.avatar else member.default_avatar.url)
        embed.set_footer(text="Powered by Ariyan", icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
