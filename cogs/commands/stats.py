...
class StatsDropdown(Select):
    def __init__(self, ctx, bot, embeds):
        ...
        super().__init__(placeholder="Ariyan Statistics.", min_values=1, max_values=1, options=options)
...
class Stats(commands.Cog):
    ...
    async def stats(self, ctx):
        ...
        loading_embed = discord.Embed(title="", description="<a:loading:1448949404185657354> **Generating Ariyan Statistics...**", color=0x00000)
        ...
        quick_embed = Embed(title="Ariyan Stats Panel", color=0x87CEEB)
        ...
        # System Embed
        system_embed = Embed(title=" System Stats", color=0x87CEEB)
        ...
        system_embed.set_footer(text="Powered by Ariyan", icon_url=ctx.me.display_avatar.url)

        # General Embed
        general_embed = Embed(title="General Stats", description=f"**Uptime**: `{uptime}`", color=0x87CEEB)
      ...
      #  general_embed.add_field(name="Latency", value=f"```[ Bot ]: {round(sh.latency * 800)}ms\n[ Database ]: 0.5ms```")
        ...
        general_embed.set_footer(text="Powered by Ariyan", icon_url=ctx.me.display_avatar.url)

        # Team Embed
        team_embed = Embed(title="Team Stats",
                           description="`There is only one person Who Made Me, Thanks To Him ❤️.`", color=0x87CEEB)
        team_embed.add_field(name="Main Owner",
                             value="**[01]. [Ariyan](https://discord.com/users/1383672814820655215)**", inline=False)
        team_embed.set_footer(text="Powered by Ariyan", icon_url=ctx.me.display_avatar.url)
        ...
        code_embed.add_field(name="Codebase Overview",
                             value=f"""**\n Files : {files}\n Lines : {lines}\n Words : {words}**""",
                             inline=False)
        code_embed.set_footer(text="Powered by Ariyan", icon_url=ctx.me.display_avatar.url)
