...
class NotifCommands(commands.Cog):
    ...
    async def setnotif(self, ctx):
        embed = discord.Embed(title="Notification Commands", color=0x87CEEB)
    ...
    async def twitch(self, ctx, role: discord.Role, channel: discord.TextChannel):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT * FROM notifications WHERE type = ?', ('twitch',)) as existing:
                ...
                if row:
                    await ctx.reply(embed=discord.Embed(title="<:zwarning:1448949627712966717>Access Denied", description="Twitch notification already set. Remove it first.", color=0x87CEEB))
                    return

            await db.execute('INSERT INTO notifications (type, role_id, channel_id) VALUES (?, ?, ?)', ('twitch', role.id, channel.id))
            await db.commit()
            await ctx.reply(embed=discord.Embed(title="<:ztick:1448951767990796298> Success", description=f"Twitch notifications set for {role.mention} in {channel.mention}.", color=0x87CEEB))
    ...
    async def youtube(self, ctx, role: discord.Role, channel: discord.TextChannel):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT * FROM notifications WHERE type = ?', ('youtube',)) as existing:
                row = await existing.fetchone()
                if row:
                    await ctx.reply(embed=discord.Embed(title="<:zwarning:1448949627712966717> Access Denied", description="YouTube notification already set. Remove it first.", color=0x87CEEB))
                    return

            await db.execute('INSERT INTO notifications (type, role_id, channel_id) VALUES (?, ?, ?)', ('youtube', role.id, channel.id))
            await db.commit()
            await ctx.reply(embed=discord.Embed(title="<:ztick:1448951767990796298> Success", description=f"YouTube notifications set for {role.mention} in {channel.mention}.", color=0x87CEEB))
    ...
    async def list(self, ctx):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT * FROM notifications') as cursor:
                ...
                if not rows:
                    await ctx.reply(embed=discord.Embed(description="No Twitch and YouTube notification channels set.", color=0x87CEEB))
                    return

            embed = discord.Embed(title="Current Notification Settings", color=0x87CEEB)
            for row in rows:
    ...
    async def reset(self, ctx):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.commit()
            await ctx.send(embed=discord.Embed(title="<:ztick:1448951767990796298> Success", description="Twitch and YouTube notifications have been reset.", color=0x00FF00))
