...
class PlatformSelectView(View):
    ...
    async def perform_search(self, source):
        ...
        if not results:
            return await self.ctx.send(embed=discord.Embed(description="No results found.", color=0x87CEEB))
...
class MusicControlView(View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        await interaction.response.send_message(
            embed=discord.Embed(description="Only members in the same voice channel as me can control the player.", color=0x87CEEB),
            ephemeral=True
        )
        return False

    @discord.ui.button(emoji="<:zunmute:1448951487970414694>", style=discord.ButtonStyle.secondary)
...
class Music(commands.Cog):
    ...
    async def inactivity_timer(self, guild):
        ...
        if len(guild.voice_channels[0].members) == 1:
            ...
            if player:
                ...
                try:
                    ended = discord.Embed(description="Bot has been disconnected due to inactivity (being idle in Voice Channel) for more than 2 minutes." , color=0x87CEEB)
                    ended.set_author(name="Inactive Timeout", icon_url=self.client.user.avatar.url)
                    ended.set_footer(text="Thanks for choosing Ariyan!")
                    support = Button(label='Support',
                                 style=discord.ButtonStyle.link,
                        url=f'https://discord.gg/PkDre8Juhp')
    ...
    async def on_track_end(self, payload: wavelink.TrackEndEventPayload):
        ...
        if not player.queue:
            if player.queue.mode == wavelink.QueueMode.loop:
                ...
                ended = discord.Embed(description="All tracks have been played, leaving the voice channel." , color=0x87CEEB)
    ...
    async def play_source(self, ctx, query):
        if not ctx.author.voice:
            await ctx.send(embed=discord.Embed(description="<:warning:1448951779353038949> you need to be in a voice channel to use this command.", color=0x87CEEB))
            return
        ...
        if vc.playing:
            if ctx.voice_client and ctx.voice_client.channel != ctx.author.voice.channel:
                await ctx.send(embed=discord.Embed(description=f"You must be connected to {ctx.voice_client.channel.mention} to play.", color=0x87CEEB))
                return
    ...
    async def handle_spotify_link(self, ctx, vc, link, type_):
        try:
            if type_ == "track":
                ...
                await ctx.send(embed=discord.Embed(description=f"<:zplus:1448951790463615038> Added **{c}** of **{playlist_length}** tracks from **playlist** **[{playlist_info['name']}](https://discord.gg/PkDre8Juhp)** to the queue.", color=0x87CEEB))
                await lmao.delete()
                ...
                await ctx.send(embed=discord.Embed(description=f"<:zplus:1448951790463615038> Added all tracks from album **[{album_info['name']}](https://discord.gg/PkDre8Juhp)** to the queue.", color=0x87CEEB))
                if not vc.playing:
                    next_track = await vc.queue.get_wait()
                    await vc.play(next_track)
                    await self.display_player_embed(vc, next_track, ctx)
    ...
    async def nowplaying(self, ctx: commands.Context):
        ...
        if not vc or not vc.playing:
            await ctx.send(embed=discord.Embed(description="No song is currently playing.", color=0x87CEEB))
            return

        if not ctx.author.voice or ctx.author.voice.channel.id != vc.channel.id:
            await ctx.send(embed=discord.Embed(description="You need to be in the same voice channel as me to use this command.", color=0x87CEEB))
            return
        ...
        embed = discord.Embed(
            title="Now Playing",
            color=0x1DB954 if source_name == "Spotify" else 0x87CEEB
        )
    ...
    async def autoplay(self, ctx: commands.Context):
        vc = ctx.voice_client
        if not vc or not vc.playing:
            await ctx.send(embed=discord.Embed(description="<:warning:1448951779353038949> No song is currently playing.", color=0x87CEEB))
            return

        if not ctx.author.voice or ctx.author.voice.channel.id != vc.channel.id:
            await ctx.send(embed=discord.Embed(description="<:warning:1448951779353038949> You need to be in the same voice channel as me to use this command.", color=0x87CEEB))
            return
        ...
        if vc:
            ...
            await ctx.send(embed=discord.Embed(description=f"<:ztick:1448951767990796298> Autoplay {'enabled' if vc.autoplay == wavelink.AutoPlayMode.enabled else 'disabled'} by {ctx.author.mention}.", color=0x87CEEB))
    ...
    async def loop(self, ctx: commands.Context):
        ...
        if vc:
            ...
            await ctx.send(embed=discord.Embed(description=f"<:ztick:1448951767990796298> Loop {'enabled' if vc.queue.mode == wavelink.QueueMode.loop else 'disabled'} by {ctx.author.mention}.", color=0x87CEEB))
        else:
            await ctx.send(embed=discord.Embed(description="I'm not connected to a voice channel.", color=0x87CEEB))
    ...
    async def pause(self, ctx: commands.Context):
        vc = ctx.voice_client
        if not vc or not vc.playing:
            await ctx.send(embed=discord.Embed(description="<:warning:1448951779353038949> No song is currently playing.", color=0x000000))
            return
        ...
        if vc and vc.playing and not vc.paused:
            await vc.pause(True)
            await vc.channel.edit(status=f"<:zmusicpause:1448951801931108413> Paused: {vc.current.title}")
            await ctx.send(embed=discord.Embed(description=f"Paused by {ctx.author.mention}.", color=0x000000))
        else:
            await ctx.send(embed=discord.Embed(description="<:warning:1448951779353038949>   Nothing is playing or already paused.", color=0x87CEEB))
    ...
    async def resume(self, ctx: commands.Context):
        ...
        if not ctx.author.voice or ctx.author.voice.channel.id != vc.channel.id:
            await ctx.send(embed=discord.Embed(description="<:icons_warning:1327829522573430864> You need to be in the same voice channel as me to use this command.", color=0x87CEEB))
            return

        if vc and vc.paused:
            await vc.pause(False)
            await vc.channel.edit(status=f"<a:music:1448966355935301643> Playing: {vc.current.title}")
            await ctx.send(embed=discord.Embed(description=f"Resumed by {ctx.author.mention}.", color=0x87CEEB))
        else:
            await ctx.send(embed=discord.Embed(description="Player is not paused.", color=0x87CEEB))
    ...
    async def skip(self, ctx: commands.Context):
        ...
        if vc.autoplay == wavelink.AutoPlayMode.enabled:
            await vc.stop()
            return await ctx.send(embed=discord.Embed(description=f"Skipped by {ctx.author.mention}.", color=0x87CEEB))


        if vc and vc.playing and not vc.queue.is_empty:
            await vc.stop()
            await ctx.send(embed=discord.Embed(description=f"Skipped by {ctx.author.mention}.", color=0x87CEEB))
        else:
            await ctx.send(embed=discord.Embed(description="<:warning:1448951779353038949> No song is playing or in the queue to skip.", color=0x87CEEB))
    ...
    async def shuffle(self, ctx: commands.Context):
        ...
        if vc and vc.queue:
            ...
            await ctx.send(embed=discord.Embed(description=f"Queue shuffled by {ctx.author.mention}.", color=0x87CEEB))
        else:
            await ctx.send(embed=discord.Embed(description="Queue is empty.", color=0x87CEEB))
    ...
    async def stop(self, ctx: commands.Context):
        ...
        if vc and player:
            ...
            await ctx.send(embed=discord.Embed(description=f"Stopped and queue cleared by {ctx.author.mention}.", color=0x87CEEB))
        else:
            await ctx.send(embed=discord.Embed(description="Nothing is playing to stop.", color=0x87CEEB))
    ...
    async def volume(self, ctx: commands.Context, level: int):
        ...
        if not vc:
            await ctx.send(embed=discord.Embed(description="<:warning:1448951779353038949> I'm not connected to a voice channel.", color=0x87CEEB))
            return
        ...
        if vc:
            if 1 <= level <= 150:
                ...
                await ctx.send(embed=discord.Embed(description=f"<:zmute:1448951435478700072> Volume set to {level}% by {ctx.author.mention}.", color=0x87CEEB))
            else:
                await ctx.send(embed=discord.Embed(description="<:warning:1448951779353038949> Volume must be between 1 and 150.", color=0x87CEEB))
        else:
            await ctx.send(embed=discord.Embed(description="Bot is not connected to a voice channel.", color=0x87CEEB))
    ...
    async def queue(self, ctx: commands.Context):
        ...
        if not vc or not vc.queue or vc.queue.is_empty:
            await ctx.send(embed=discord.Embed(description="<:warning:1448951779353038949> The queue is currently empty.", color=0x87CEEB))
            return
    ...
    async def clearqueue(self, ctx: commands.Context):
        ...
        if not vc or not vc.queue or vc.queue.is_empty:
            await ctx.send(embed=discord.Embed(description="<:warning:1448951779353038949> No Queue to clear.", color=0x87CEEB))
            return
        ...
        if vc and vc.queue:
            ...
            await ctx.send(embed=discord.Embed(description="No queue to clear.", color=0x87CEEB))
    ...
    async def replay(self, ctx: commands.Context):
        ...
        if not vc or not vc.playing:
            await ctx.send(embed=discord.Embed(description="<:warning:1448951779353038949> I'm not connected to any voice channel.", color=0x87CEEB))
            return
        ...
        if vc and vc.playing:
            ...
            await ctx.send(embed=discord.Embed(description="No track is currently playing.", color=0x87CEEB))
    ...
    async def join(self, ctx: commands.Context):
        if ctx.author.voice:
            ...
            await ctx.send(embed=discord.Embed(description="You need to join a voice channel first.", color=0x87CEEB))
    ...
    async def disconnect(self, ctx: commands.Context):
        vc = ctx.voice_client
        if not vc:
            await ctx.send(embed=discord.Embed(description="<:icons_warning:1327829522573430864>  I'm not connected to any voice channel.", color=0x87CEEB))
            return
        ...
        if vc:
            ...
            await ctx.send(embed=discord.Embed(description="Bot is not connected to any voice channel.", color=0x87CEEB))
    ...
    async def seek(self, ctx: commands.Context, percentage: int):
        if not 1 <= percentage <= 100:
            await ctx.send(embed=discord.Embed(description="Please provide a percentage between 1 and 100.", color=0x87CEEB))
            return
    ...
    async def on_wavelink_track_start(self, payload: wavelink.TrackStartEventPayload):
        ...
        if voice_channel:
            await voice_channel.edit(status=f"<a:music:1448966355935301643> Playing: {track.title}")  # type: ignore
