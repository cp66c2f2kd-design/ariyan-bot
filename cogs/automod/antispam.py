...
class AntiSpam(commands.Cog):
    ...
    async def log_action(self, guild, user, channel, action, reason):
        ...
        if log_channel_id and log_channel_id[0]:
            ...
            if log_channel:
                embed = discord.Embed(title="Automod Log: Anti-Spam", color=0x87CEEB)
    ...
    async def on_message(self, message):
        ...
        if len(user_messages) > self.spam_threshold:
            ...
            try:
                ...
                simple_embed = discord.Embed(title="Automod Anti-Spam", color=0x87CEEB)
                simple_embed.description = f"<:ztick:1448951767990796298> | {user.mention} has been successfully **{action_taken}** for **Spamming.**"
