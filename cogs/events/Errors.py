...
class Errors(Cog):
  ...
  async def on_command_error(self, ctx: Context, error):
    ...
    if isinstance(error, commands.NoPrivateMessage):
      embed = discord.Embed(color=0x87CEEB, description="You can't use my commands in DMs.")
    ...
    if isinstance(error, commands.CommandOnCooldown):
      embed = discord.Embed(color=0x87CEEB, description=f"**{ctx.author.mention} Couldown is here Bro Tryy commands in {error.retry_after:.2f} seconds**.")
    ...
    if isinstance(error, commands.MaxConcurrencyReached):
      embed = discord.Embed(color=0x87CEEB, description=f"{ctx.author.mention} This command is already in progress. Please let it finish and try again afterward.")
    ...
    if isinstance(error, commands.MissingPermissions):
      ...
      embed = discord.Embed(color=0x87CEEB, description=f"**Ops! You don't have {fmt} Permission to run the {ctx.command.name} command!**")
